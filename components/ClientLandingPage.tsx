...
import { Hexagon, CheckCircle, ArrowRight, Zap, TrendingUp, ShieldCheck, DollarSign, Clock, LayoutDashboard, Sparkles, Smartphone, Award, MapPin, Phone, Mail } from 'lucide-react';
      ...
      <nav className="fixed w-full z-50 bg-slate-950/80 backdrop-blur-xl border-b border-white/5 transition-all">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <span className="text-2xl font-black tracking-tighter text-white uppercase">{branding?.name.split(' ')[0] || 'Nexus'}<span className="text-emerald-500">{branding?.name.split(' ')[1] || 'Capital'}</span></span>
          </div>
          ...
          <div className="flex gap-6 items-center">
            ...
            <button 
              ...
              className="px-8 py-3 text-xs font-black uppercase tracking-widest bg-emerald-500 text-slate-950 rounded-xl hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20 transform active:scale-95"
            >
              Apply Now
            </button>
          </div>
         ...
         <div className="max-w-7xl mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-16">
              <div className="col-span-1 md:col-span-2 space-y-8">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xl font-black text-white uppercase tracking-tighter">{branding?.name || 'Nexus Capital'}</span>
                </div>
                <div className="space-y-4">
                  {branding?.physicalAddress && (
                    <div className="flex items-start gap-4 text-xs text-slate-500 font-medium">
                  ...
                  {branding?.contactPhone && (
                    <div className="flex items-center gap-4 text-xs text-slate-500 font-medium">
                      <Phone size={18} className="text-slate-700 shrink-0" />
                      <span>{branding.contactPhone}</span>
                    </div>
                  )}
                  {branding?.contactEmail && (
                    <div className="flex items-center gap-4 text-xs text-slate-500 font-medium">
                      <Mail size={18} className="text-slate-700 shrink-0" />
              ...
              <div>
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-700 mb-6">Navigation</h4>
                <ul className="space-y-3 text-xs font-bold text-slate-500">
                  <li><a href="#how-it-works" className="hover:text-emerald-400 transition-colors">Infrastructure</a></li>
                  <li><a href="#benefits" className="hover:text-emerald-400 transition-colors">Yield</a></li>
                  <li><button onClick={() => onNavigate(ViewMode.LOGIN)} className="hover:text-emerald-400 transition-colors">Member Access</button></li>
                </ul>
              </div>

              <div>
                <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-700 mb-6">Security</h4>
                <ul className="space-y-3 text-xs font-bold text-slate-500">
            ...
            <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-600">
                 © 2024 {branding?.name || 'Nexus OS'} • Bank-Grade Security
              </div>
              <div className="flex gap-8 text-[10px] font-black uppercase tracking-widest text-slate-400">
                 <button onClick={() => onNavigate(ViewMode.LANDING)} className="hover:text-white transition-colors opacity-40">Broker Access</button>
              </div>
            </div>
         </div>
      </footer>
