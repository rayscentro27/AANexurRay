...
const ReputationManager: React.FC = () => {
  ...
  const [businessLocInput, setBusinessLocInput] = useState('');
  ...
  const avgRating = verifiedProfile?.rating || (reviews.reduce((acc, r) => acc + r.rating, 0) / totalReviews).toFixed(1);
  const nps = 72; 
  ...
  const handleConnectGoogle = async () => {
    ...
    try {
        // 1. Verify Profile exists on Maps using Search tool
        const profile = await geminiService.verifyBusinessPresence(businessNameInput, businessLocInput);
        
        if (profile.exists) {
            setVerifiedProfile(profile);
            // 2. Mock fetching the live reviews
            const newMockReviews = await geminiService.generateMockGoogleReviews(profile.officialName || businessNameInput);
            if (newMockReviews.length > 0) {
                // Filter logic for auto-pilot if it were immediate: only process good reviews
                const analyzedReviews = newMockReviews.map(r => {
                    if (autoPilot && r.rating >= 4) {
                        // In real app, we'd trigger the AI here
                    }
                    return r;
                });

                setReviews(prev => [...analyzedReviews, ...prev]);
                const analysis = await geminiService.analyzeReviewSentiment(newMockReviews);
                setSentimentData(analysis);
                setGoogleConnected(true);
                setIsConnectModalOpen(false);
                alert("Google Business Profile connected! 3 verified reviews synchronized.");
            }
        } else {
            alert("No matching Google Business Profile found. Please check name/location.");
        }
    } catch (e) {
        console.error(e);
                ...
                <div className="bg-slate-950 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-[10px] font-black text-slate-400 mt-6 uppercase tracking-widest">
                            {verifiedProfile?.reviewCount || totalReviews} verified mentions detected
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-8 rounded-[2.5rem] shadow-sm flex flex-col justify-between group hover:border-blue-300 transition-all">
                    ...
                    <div className="mt-8">
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                            <div className="bg-blue-600 h-full shadow-[0_0_10px_rgba(37,99,235,0.4)]" style={{ width: `${nps}%` }}></div>
                        </div>
