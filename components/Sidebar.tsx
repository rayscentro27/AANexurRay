...
import { 
  ...
  Facebook, Instagram, Linkedin, Receipt, Award
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, pendingDocCount = 0, onLogout, branding }) => {
  ...
  const isStaff = user?.role === 'salesperson' || user?.role === 'supervisor';
      ...
      <div className={`fixed top-0 left-0 h-screen w-64 bg-slate-950 text-white flex flex-col shadow-2xl z-50 transition-transform duration-300 ease-in-out border-r border-white/10 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 py-6 px-4 space-y-8 overflow-y-auto no-scrollbar scroll-smooth">
          ...
          <SidebarSection label="Global Ops">
            ...
            {(isAdmin || isSupervisor) && <SidebarItem id={ViewMode.INBOX} label="Unified Inbox" icon={Inbox} currentView={currentView} onViewChange={handleNav} badge={3} />}
            <SidebarItem id={ViewMode.CALENDAR} label="Smart Calendar" icon={Calendar} currentView={currentView} onViewChange={handleNav} />
            {isStaff && <SidebarItem id={ViewMode.TRAINING} label="Staff Academy" icon={GraduationCap} currentView={currentView} onViewChange={handleNav} badgeColor="bg-emerald-500" />}
          </SidebarSection>
          ...
          {(isAdmin || isSupervisor) && (
            <SidebarSection label="Capital Lab">
                <SidebarItem id={ViewMode.INVOICING} label="Revenue Hub" icon={Receipt} currentView={currentView} onViewChange={handleNav} />
          ...
          <SidebarSection label="System">
            {isMasterAdmin && <SidebarItem id={ViewMode.SITEMAP} label="Core Sitemap" icon={List} currentView={currentView} onViewChange={handleNav} />}
            {isMasterAdmin && <SidebarItem id={ViewMode.SETTINGS} label="OS Settings" icon={Settings} currentView={currentView} onViewChange={handleNav} badge={disconnectedSocials > 0 ? disconnectedSocials : undefined} badgeColor="bg-red-500" />}
          </SidebarSection>
        </nav>
