...
import { 
    ...
    ArrowUpRight, Users, MessageCircle, BarChart3, TrendingUp, RefreshCw
} from 'lucide-react';
...
const GlobalFundPulse: React.FC<GlobalFundPulseProps> = ({ contacts }) => {
  const activeDialerCount = 2; // Mock live count
  const liveSessions = 4; // Mock live count
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-slate-950 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-emerald-400 text-[9px] font-black uppercase tracking-widest mb-4 flex items-center gap-2">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></div> Floor Velocity
            </p>
            <h3 className="text-3xl font-black text-white tracking-tighter">8 ACTIVE OPS</h3>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-4">2 Dialer • 4 Combat • 2 Intake</p>
        </div>
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {[
                            ...
                            { name: 'Mike Closer', avatar: 'MC', protocol: 'Subject Intake', duration: '04m 30s', status: 'IDLE', color: 'text-slate-400' },
                        ].map((agent, i) => (
         ...
         <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-indigo-600 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                ...
                <p className="text-sm text-indigo-100 font-medium mb-8 leading-relaxed italic">
                    "Detected stale deals in John Doe's queue. Triggering auto-escalation to Power Dialer."
                </p>
                <button className="w-full py-4 bg-white text-indigo-600 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl transform active:scale-95 transition-all">
                    Confirm Action
                </button>
            </div>
         </div>
