...
const AdminSetupWizard: React.FC<AdminSetupWizardProps> = ({ onNavigate, branding, onUpdateBranding }) => {
  ...
  const [dbConfig, setDbConfig] = useState({
    url: BACKEND_CONFIG.supabase.url || '',
    key: BACKEND_CONFIG.supabase.anonKey || ''
  });
  ...
  const testSupabaseConnection = async () => {
    if (!dbConfig.url || !dbConfig.key) {
        setDbError('Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Netlify, then redeploy.');
        setDbStatus('error');
        return;
    }
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: 'Set Supabase env vars in Netlify and apply the schema to enable live mode.',
      icon: Database,
      ...
      component: (
        <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input 
                                ...
                                value={dbConfig.url}
                                readOnly
                                className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-xs font-mono text-slate-400 outline-none"
                            />
                        </div>
                        <div>
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Anon Key (Netlify)</label>
                            <input 
                                type="password" 
           ...
           <div className="p-4 bg-white/5 border border-white/10 rounded-xl text-[10px] text-slate-300 font-medium leading-relaxed">
             <p className="mb-2 font-black uppercase tracking-widest text-slate-400">Live Mode Checklist</p>
             <ol className="list-decimal list-inside space-y-1">
               <li>Create a Supabase project.</li>
               <li>Run the SQL in <code className="bg-white/10 px-1 rounded">supabase/migrations/001_init.sql</code>.</li>
               <li>Set <code className="bg-white/10 px-1 rounded">VITE_SUPABASE_URL</code> and <code className="bg-white/10 px-1 rounded">VITE_SUPABASE_ANON_KEY</code> in Netlify.</li>
               <li>Redeploy, then click Validate Config.</li>
             </ol>
           </div>
